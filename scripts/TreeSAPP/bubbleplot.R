...
make_plot <- function(df){
  ...
  pl <- ggplot(data = df, aes(x = depth, y = species_nums, color = depth)) + 
    geom_point(aes(size = abundance)) +
    scale_color_manual(values = wes_palette(9, name = "Cavalcanti1", type = "continuous"), name = "") +
    theme_classic() +
    scale_size(range = c(1, 7)) +
    scale_y_continuous(labels =  as.vector(df$species)[1:5],
                       sec.axis = dup_axis(labels =  as.vector(df$class)[1:5])) +
    scale_size_continuous(breaks = c(100, 300, 500, 750, 1000))+
    xlab(" ") +
    ylab(" ") +
...
NifD_df <- main_func("NifD")
NirK_df <- main_func("NirK")
NirS_df <- main_func("NirS")
NorB_df <- main_func("NorB")
NorC_df <- main_func("NorC")
NosZ_df <- main_func("NosZ")
NxrA_df <- main_func("NxrA")
NxrB_df <- main_func("NxrB")

NapA_plot <- make_plot(NapA_df)
NifD_plot <- make_plot(NifD_df)
NirK_plot <- make_plot(NirK_df)
NirS_plot <- make_plot(NirS_df)
NorB_plot <- make_plot(NorB_df)
NorC_plot <- make_plot(NorC_df)
NosZ_plot <- make_plot(NosZ_df)
NxrA_plot <- make_plot(NxrA_df)
NxrB_plot <- make_plot(NxrB_df)

fig_height <- 35
fig_width <- 9

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NapA_plot.pdf", sep = "/")
NapA_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NifD_plot.pdf", sep = "/")
NifD_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NirK_plot.pdf", sep = "/")
NirK_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NirS_plot.pdf", sep = "/")
NirS_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NorB_plot.pdf", sep = "/")
NorB_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NorC_plot.pdf", sep = "/")
NorC_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NosZ_plot.pdf", sep = "/")
NosZ_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NxrA_plot.pdf", sep = "/")
NxrA_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/bubble/NxrB_plot.pdf", sep = "/")
NxrB_plot
ggsave(figure_path, width = fig_height, height = fig_width, units = "cm")
