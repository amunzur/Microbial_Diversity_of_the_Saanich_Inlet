...
idx <- grep("METABAT", cc_main$bin) # find bins with the metabat 
cc_metabat <- cc_main[idx, ] # subset 

idx <- grep("METABAT", cc_taxa_arc$bin)
cc_taxa_arc <- cc_taxa_arc[idx, ]

idx <- grep("METABAT", cc_taxa_bac$bin)
cc_taxa_bac <- cc_taxa_bac[idx, ]

# some bins are missing in cc_taxa_bac, transfer those from cc_main
idx <- which(!(cc_metabat$bin %in% cc_taxa_bac$bin))
bin_names <- cc_metabat$bin[idx]
taxa_names <- rep("NA", length(bin_names))
df <- as.data.frame(cbind(bin_names, taxa_names))
names(df) <- names(cc_taxa_bac)
cc_taxa_bac <- rbind(cc_taxa_bac, df)

# separate out the taxa information
ranks <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
arc_bac_combined <- rbind(cc_taxa_arc, cc_taxa_bac) %>% 
...
p1 <- ggplot(data = mags_combined, aes(x = Completeness, y = Contamination, color = Phylum)) + 
  ...
  theme_minimal() + 
  theme_asli +
  ggtitle("METABAT completion - contamination plot showing phylum") +
  xlim(0, 100) + 
  ylim(0, 10) 
...
p2 <- ggplot(data = mags_combined, aes(x = Completeness, y = Contamination, color = Order)) + 
  ...
  theme_minimal() + 
  theme_asli +
  ggtitle("METABAT completion - contamination plot showing order") +
  xlim(0, 100) + 
  ylim(0, 10)

figure_path <- paste("../FINAL_PROJ/figures/SUPP/METABAT_cc_phylum.pdf", sep = "/")
p1
ggsave(figure_path, width = 25, height = 15, units = "cm")

figure_path <- paste("../FINAL_PROJ/figures/SUPP/METABAT_cc_order.pdf", sep = "/")
p2
ggsave(figure_path, width = 25, height = 15, units = "cm")
